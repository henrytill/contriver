open Contriver

(* Environment Handling *)

let get_var env var =
  try
    Result.Ok (List.assoc var !env)
  with Not_found ->
    Result.Error (UnboundVar ("Getting an unbound variable", var))

let set_var env var value =
  try
    let _ = List.assoc var !env in
    env := (var, value) :: List.remove_assoc var !env;
    Result.Ok value
  with Not_found ->
    Result.Error (UnboundVar ("Setting an unbound variable", var))

let define_var env var value =
  env := (var, value) :: List.remove_assoc var !env;
  Result.Ok value

let bind_vars bindings env =
  List.iter (fun (var, value) ->
      env := (var, value) :: List.remove_assoc var !env)
    bindings;
  env

let fresh_context env =
  let bindings = ref !env in
  bindings


(* Unpackers *)

type 'a unpacker = lisp_value -> 'a throws_error

let unpack_num : int unpacker = function
  | Number n -> Result.Ok n
  | x        -> Result.Error (TypeMismatch ("number", x))

let unpack_bool : bool unpacker = function
  | Bool b -> Result.Ok b
  | x      -> Result.Error (TypeMismatch ("bool", x))

let unpack_str : string unpacker = function
  | String s -> Result.Ok s
  | x        -> Result.Error (TypeMismatch ("string", x))

let unpack_equals a b (unpacker : 'a unpacker) =
  unpacker a >>= fun upa ->
  unpacker b >>= fun upb ->
  Result.Ok (upa = upb)


(* Unary Operations *)

let unary_op f = function
  | [v] -> Result.Ok (f v)
  | x   -> Result.Error (NumArgs (1, x))

let symbolp = function
  | Atom _ -> Bool true
  | _      -> Bool false

let numberp = function
  | Number _ -> Bool true
  | _        -> Bool false

let stringp = function
  | String _ -> Bool true
  | _        -> Bool false

let boolp = function
  | Bool _ -> Bool true
  | _      -> Bool false

let listp = function
  | List _       -> Bool true
  | DottedList _ -> Bool true
  | _            -> Bool false

let symbol_to_string = function
  | Atom s -> String s
  | _      -> String ""

let string_to_symbol = function
  | String s -> Atom s
  | _        -> Atom ""


(* Binary Operations *)

let numeric_bin_op op = function
  | [] as param ->
      Result.Error (NumArgs (2, param))
  | hd :: tl    ->
      List.fold_left
        (fun acc c ->
           match acc, c with
           | Result.Ok (Number x), Number y -> Result.Ok (Number (op x y))
           | _,                    _        -> raise (Failure "numeric_bin_op: this should never happen"))
        (Result.Ok hd)
        tl

let bool_bin_op unpacker op = function
  | [a; b] ->
      unpacker a >>= fun l ->
      unpacker b >>= fun r ->
      Result.Ok (Bool (op l r))
  | x ->
      Result.Error (NumArgs (1, x))

let num_bool_bin_op =
  bool_bin_op unpack_num

let str_bool_bin_op =
  bool_bin_op unpack_str

let bool_bool_bin_op =
  bool_bin_op unpack_bool


(* List Helpers *)

let rec last = function
  | []       -> None
  | hd :: [] -> Some hd
  | _  :: tl -> last tl

let rec drop n xs =
  match xs with
  | []      -> []
  | _ :: tl -> if n = 0 then xs else drop (n - 1) tl

let zip xs ys =
  let rec loop xs ys acc =
    match (xs, ys) with
    | [], _ | _, []        -> List.rev acc
    | (l :: ls), (r :: rs) -> loop ls rs ((l, r) :: acc)
  in
  loop xs ys []


(* List Primitives *)

let car = function
  | [List (x :: _)]          -> Result.Ok x
  | [DottedList (x :: _, _)] -> Result.Ok x
  | [x]                      -> Result.Error (TypeMismatch ("pair", x))
  | x                        -> Result.Error (NumArgs (1, x))

let cdr = function
  | [List (_ :: xs)]               -> Result.Ok (List xs)
  | [DottedList (_ :: s :: xs, x)] -> Result.Ok (DottedList (s :: xs, x))
  | [DottedList ([_], x)]          -> Result.Ok x
  | [x]                            -> Result.Error (TypeMismatch ("pair", x))
  | x                              -> Result.Error (NumArgs (1, x))

let cons = function
  | [h; List []]            -> Result.Ok (List [h])
  | [h; List xs]            -> Result.Ok (List (h :: xs))
  | [h; DottedList (xs, x)] -> Result.Ok (DottedList (h :: xs, x))
  | [h; x]                  -> Result.Ok (DottedList ([h], x))
  | h                       -> Result.Error (NumArgs (2, h))

let eq_pair f (x, y) : bool =
  match f [x; y] with
  | Result.Error _     -> false
  | Result.Ok (Bool v) -> v
  | Result.Ok z        -> raise (Failure "eq_pair: this should never happen")

let eq_list f : lisp_value list -> lisp_value throws_error  = function
  | [List x; List y] -> Result.Ok (Bool (List.length x = List.length y && List.for_all (eq_pair f) (zip x y)))
  | _                -> raise (Failure "eq_list: this should never happen")

let rec eqv : lisp_value list -> lisp_value throws_error = function
  | [Bool x; Bool y]                         -> Result.Ok (Bool (x = y))
  | [Number x; Number y]                     -> Result.Ok (Bool (x = y))
  | [Float x; Float y]                       -> Result.Ok (Bool (x = y))
  | [Ratio (a, b); Ratio (c, d)]             -> Result.Ok (Bool (let open Num in eq_num (div_num a b) (div_num c d)))
  | [Complex x; Complex y]                   -> Result.Ok (Bool (x = y))
  | [String x; String y]                     -> Result.Ok (Bool (x = y))
  | [Atom x; Atom y]                         -> Result.Ok (Bool (x = y))
  | [(List _) as l1; (List _) as l2]         -> eq_list eqv [l1; l2]
  | [DottedList (xs, x); DottedList (ys, y)] -> eq_list eqv [List (xs @ [x]); List (ys @ [y])]
  | [_; _]                                   -> Result.Ok (Bool false)
  | x                                        -> Result.Error (NumArgs (2, x))


(* Eval and friends *)

exception Eval_error of lisp_error

exception Found_match

let contains key xs =
  try
    List.iter (fun x ->
        match eqv [key; x] with
        | Result.Ok (Bool false) -> ()
        | Result.Ok (Bool true)  -> raise Found_match
        | Result.Ok _            -> raise (Failure "find_match: this should never happen")
        | Result.Error e         -> raise (Eval_error e))
      xs;
    Result.Ok (Bool false)
  with
  | Found_match   -> Result.Ok (Bool true)
  | Eval_error e  -> Result.Error e

let make_func varargs env params body =
  let ps = List.map show_lisp_value params in
  Result.Ok (Func { params  = ps; varargs; body; closure = !env; })

let make_normal_func =
  make_func None

let make_varargs x =
  make_func (Some (show_lisp_value x))

let isNone = function
  | Some _ -> false
  | None   -> true

let primitives =
  [ ("+",              numeric_bin_op (+))
  ; ("-",              numeric_bin_op (-))
  ; ("*",              numeric_bin_op ( * ))
  ; ("/",              numeric_bin_op (/))
  ; ("mod",            numeric_bin_op (mod))
  ; ("symbol?",        unary_op symbolp)
  ; ("number?",        unary_op numberp)
  ; ("string?",        unary_op stringp)
  ; ("bool?",          unary_op boolp)
  ; ("list?",          unary_op listp)
  ; ("symbol->string", unary_op symbol_to_string)
  ; ("string->symbol", unary_op string_to_symbol)
  ; ("=",              num_bool_bin_op (==))
  ; ("<",              num_bool_bin_op (<))
  ; (">",              num_bool_bin_op (>))
  ; ("/=",             num_bool_bin_op (<>))
  ; (">=",             num_bool_bin_op (>=))
  ; ("<=",             num_bool_bin_op (<=))
  ; ("&&",             bool_bool_bin_op (&&))
  ; ("||",             bool_bool_bin_op (||))
  ; ("string=?",       str_bool_bin_op (==))
  ; ("string<?",       str_bool_bin_op (<))
  ; ("string>?",       str_bool_bin_op (>))
  ; ("string<=?",      str_bool_bin_op (<=))
  ; ("string>=?",      str_bool_bin_op (>=))
  ; ("cons",           cons)
  ; ("car",            car)
  ; ("cdr",            cdr)
  ; ("eqv?",           eqv)
  ; ("eq?",            eqv)
  ]

let primitive_bindings =
  let bindings = ref [] in
  let make_prim_func (var, func) = var, PrimitiveFunc func in
  let all_primitives = List.map make_prim_func primitives in
  bindings := all_primitives @ !bindings;
  bindings

let rec eval env = function
  | String _ | Number _ | Bool _ as v ->
      Result.Ok v
  | Atom x ->
      get_var env x
  | List [Atom "quote"; v] ->
      Result.Ok v
  | List [Atom "if"; predicate; consequent; alternate] ->
      if_exp env predicate consequent alternate
  | List (Atom "cond" :: clauses) ->
      cond_exp env clauses
  | List (Atom "case" :: key :: clauses) ->
      case_exp env key clauses
  | List [Atom "set!";   Atom var; form] ->
      eval env form >>= set_var env var
  | List (Atom "let" :: List bindings :: body) ->
      let_exp env bindings body
  | List [Atom "define"; Atom var; form] ->
      eval env form >>= define_var env var
  | List (Atom "define" :: List (Atom var :: params) :: body) ->
      make_normal_func env params body >>= define_var env var
  | List (Atom "define" :: DottedList ((Atom var :: params), varargs) :: body) ->
      make_varargs varargs env params body >>= define_var env var
  | List (Atom "lambda" :: List params :: body) ->
      make_normal_func env params body
  | List (Atom "lambda" :: DottedList (params, varargs) :: body)->
      make_varargs varargs env params body
  | List (Atom "lambda" :: (Atom _ as varargs) :: body) ->
      make_varargs varargs env [] body
  | List [Atom "eval" ; value] ->
      eval_expr env value
  | List (f :: args) ->
      eval env f         >>= fun func ->
      eval_list env args >>= fun arg_vals ->
      apply env func arg_vals
  | bad_form ->
      Result.Error (BadSpecialForm ("Unrecognized special form", bad_form))

and eval_expr env expr =
  match eval env expr with
  | Result.Ok (List _ as v) -> eval env v
  | x                -> x

and eval_list env xs =
  try
    List.map (fun x ->
        match eval env x with
        | Result.Ok v    -> v
        | Result.Error e -> raise (Eval_error e))
      xs
    |> (fun xs -> Result.Ok xs)
  with Eval_error e ->
    Result.Error e

and eval_body body env =
  let last_wrapper x = last x |> option_to_result (NumArgs (1, [List []])) in
  eval_list env body >>= last_wrapper

and if_exp env predicate consequent alternate =
  let g = function
    | Bool true  -> eval env consequent
    | Bool false -> eval env alternate
    | x          -> Result.Error (TypeMismatch ("bool", x))
  in
  eval env predicate >>= g

and cond_exp env = function
  | [List [Atom "else"; consequent]]   -> eval env consequent
  | List [predicate; consequent] :: xs ->
      let g = function
        | Bool true  -> eval env consequent
        | Bool false -> cond_exp env xs
        | x          -> Result.Error (TypeMismatch ("bool" ,x))
      in
      eval env predicate >>= g
  | x -> Result.Error (NumArgs (1, x))

and case_exp env key = function
  | List (Atom "else" :: then_body) :: _ ->
      eval_body then_body env
  | List (List datums :: then_body) :: clauses ->
      eval env key           >>= fun result      ->
      contains result datums >>= fun found_match ->
      if found_match = Bool true then
        eval_body then_body env
      else
        case_exp env key clauses
  | [] ->
      Result.Error Undefined
  | clauses ->
      Result.Error (BadSpecialForm ("Ill-formed case expression", List (Atom "case" :: key :: clauses)))

and let_exp env bindings body =
  let collect_let_bindings =
    List.fold_left
      (fun acc -> function
         | List [Atom var; value] -> (var, value) :: acc
         | _  -> raise (Eval_error (BadSpecialForm ("ill-formed let expression", List bindings))))
      []
  in
  let eval_values =
    List.map
      (fun (var, value) ->
         match eval env value with
         | Result.Ok v    -> (var, v)
         | Result.Error e -> raise (Eval_error e))
  in
  let let_bindings = collect_let_bindings bindings |> eval_values in
  try
    fresh_context env
    |> bind_vars let_bindings
    |> eval_body body
  with Eval_error e ->
    Result.Error e

and apply env func args =
  match func with
  | PrimitiveFunc f ->
      f args
  | Func { params; varargs; body; closure } ->
      let num = List.length in
      if (num params) <> (num args) && isNone varargs then
        Result.Error (NumArgs (num params, args))
      else
        begin
          let remaining_args = drop (num params) args in
          let bind_varargs args env =
            match args with
            | Some arg_name -> bind_vars [arg_name, List remaining_args] env
            | None          -> env
          in
          fresh_context env
          |> bind_vars closure
          |> bind_vars (zip params args)
          |> bind_varargs varargs
          |> eval_body body
        end
  | x ->
      Result.Error (TypeMismatch ("func", x))
